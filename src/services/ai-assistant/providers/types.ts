 ...
 *
 * It is deliberately the only place in this service that can be
 * non-deterministic. Everything downstream — merchant resolution, request
 * matching, status, confidence clamping — is plain code, so swapping the
 * provider cannot weaken any security property.
 */
